...
async function processBookTags(text, baseUrl) {
    ...
    while (match = regex.exec(text)) {
        ...
        setTimeout(() => loadBookCard(bookId, cardId, baseUrl, showTags, showBio), 0);
...
async function loadBookCard(bookId, elementId, baseUrl, showTags, showBio) {
    ...
    try {
        ...
        const response = await fetch(`${baseUrl}/api/novel/detail.php?id=${bookId}`);
...
async function renderMarkdown(text, baseUrl) {
    return marked.parse(processFoldTags(processSpoiler(await processBookTags(text, baseUrl))));
}
...
async function getComment(data, baseUrl) {
    ...
    async function renderComment(comment, isReply = false, depth = 0) {
        ...
        const contentHtml = await renderMarkdown(comment.content, baseUrl);
        ...
        if (replies.length > 0) {
            if (depth === 0) {
                ...
                const replyItems = await Promise.all(replies.map(reply => renderComment(reply, true, depth + 1)));
                repliesHtml = `
    ...
    const commentItems = await Promise.all(comments.map(comment => renderComment(comment, false, 0)));
    return commentItems.join('');
}

async function getReview(baseUrl, bookName, chapterName, bookId, chapterId, detailUrl, coverUrl) {
    ...
    try {
        ...
        const firstResponse = await fetch(`${apiUrl}&page=1`);
        const firstData = await firstResponse.json();
        allCommentsHtml += await getComment(firstData, baseUrl);
        
        const totalPages = firstData.pages || 1;
        for (let page = 2; page <= totalPages; page++) {
            const response = await fetch(`${apiUrl}&page=${page}`);
            const data = await response.json();
            allCommentsHtml += await getComment(data, baseUrl);
        }
...
function initComments(config) {
    ...
    if (baseUrl && bookId) {
        getReview(baseUrl, bookName, chapterName, bookId, chapterId, detailUrl, coverUrl);
